...
export const StreamData = createSlice({
  ...
  reducers: {
    addFullVehData: (state, { payload }) => {
      ...
      const FullVehData = JSON.parse(JSON.stringify(payload)).forEach((l) => {
        const status = CalcVstatus(l);
        l.VehicleStatus = status;
      });
      state.VehTotal = CalcVehTotal([...payload]);
      state.VehFullData = [...payload];
