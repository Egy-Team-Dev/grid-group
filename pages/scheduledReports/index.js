...
const ScheduledReports = () => {
  ...
  const fetchAllScheduledReportsData = useCallback(async () => {
  ...
  const onDelete = async () => {
    ...
    try {
  const deletedSelectedReport =   await deleteReport(deleteSelectedReportString);
      if(deletedSelectedReport.message=="deleted"){
        // for one selected item
        if(rowsSelected.length===1 || typeof deleteSelectedReportString =="string"){
          const filteredData = DataTable?.filter((report,idx) => {
            return  typeof deleteSelectedReportString==="object"? deleteSelectedReportString[idx] !== report._id: deleteSelectedReportString !== report._id
           })
           setDataTable(filteredData);

        }else{
              // for multi selected deleted items
          let DataAfterMultiSelectedDeletedItems = DataTable.filter(item => !deleteSelectedReportString.includes(item._id));
...
console.log({deleteSelectedReport});
