...
const Reports = ({ dataSideBar }) => {
  ...
  const fetchReports = async (
    ...
    try {
      ...
      toast.error("Errorss: " + error.response.data?.message);
    }
  };
  ...
  const ShowReports = async (Show, name, fullSelectedReportData) => {
    if (Show === "Show") {
      ...
      if (vehChecked.length === 0) {
        toast.error("Please Select At Least one Vehicle");
        return;
      }
      // check if user clicked on the show button
      if (reportsDataSelected.vehChecked.length > 0) {
        // check if there is a vehChecked
        setVehiclesError(""); // reset setVehiclesError
  ...
  const handleTap = (name, id, api, startDate, endDate, vehCh) => {
    setReportApi(api);
    setVehChecked(vehCh);
    // const apiFormat = name.split('_').join('')
    setFullSelectedReportData((prev) => ({
      ...prev,
      api,
      name,
