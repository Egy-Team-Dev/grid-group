        ...
        if (lat == 0 || lng == 0) {
          let res = await axios.get(
            `vehicles/historyplayback?VehID=${
              query?.VehID
            }&pageNumber=${1}&pageSize=${1}&strDate=${StrDate}&endDate=${EndDate}&mode=1`
          );
          if (res.status === 200) {
        ...
        await axios
          ...
          .then(({ data }) => {
            if (data?.historysteps) {
              ...
              //   ...s,
              //   ID: s.ID ?? s?._id,
              //   StrEvent: !s?._id ? s.StrEvent : s.IsTrip ? "Trip" : "Parked",
              //   Endvent: !s?._id ? s.Endvent : s.IsTrip ? "Parked" : "Trip",
              //   IsIdle: !s?._id ? s.StrEvent == "Trip" && !s.Distance : s.IsTrip && !s.Distance,
              //   StrDate: s.StrDate.replace("Z",""),
              //   EndDate: s.EndDate.replace("Z",""),
              //   maxIdle: 60 * 10,
              //   maxIdleSince: 0,
              //   excessiveIdle: false,
              // }));
